...
class OffboardControl {
 public:
  OffboardControl() : offboard_nh_("~") {
    mavros_setpoint_pos_pub_ =
        offboard_nh_.advertise<mavros_msgs::PositionTarget>(
            "/mavros/setpoint_raw/local", 10);
    mount_control_pub_ = offboard_nh_.advertise<mavros_msgs::MountControl>(
        "/mavros/mount_control/command", 1);

    mavros_setpoint_local_pos_pub_ =
        offboard_nh_.advertise<geometry_msgs::PoseStamped>(
            "/mavros/setpoint_position/local", 10);

    actuator_setpoint_pub_ =
        offboard_nh_.advertise<mavros_msgs::ActuatorControl>(
  ...
  void send_attitude_setpoint(const Eigen::Vector3d& _AttitudeReference,
                              float thrust_sp);
  void send_attitude_rate_setpoint(const Eigen::Vector3d& attitude_rate_sp,
                                   float thrust_sp);
  void send_mount_control_command(const Eigen::Vector3d& mount_sp);
  void send_body_velxy_posz_setpoint(const Eigen::Vector3d& vel_sp,
                                     float desire_z);
  void send_body_velxy_posz_yaw_setpoint(const Eigen::Vector3d& vel_sp,
                                         float desire_z, float yaw_sp);
...
void offboard_control::OffboardControl::send_body_velyz_setpoint(
  ...
  // bit 7: ax, bit 8: ay, bit 9: az, bit 10: is_force_sp,
  // bit 11: yaw, bit 12: yaw_rate
  // Bit 10 should set to 0, meaning not force sp.
